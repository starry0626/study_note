...
相较于传统的全部thinking的训练, 本文所采用的方式也可以取得较明显的优势
...
这是数据集的核心价值所在。原始数据集通常只有答案没有推理过程，或者推理过程质量不高。作者使用Gemini-2.5-pro，在给定正确答案的前提下，生成带有时间戳的详细推理步骤。
1. 质量过滤 (Filtering)
...
传统的纯文本CoT. SFT+GRPO训练, 提出了强化模型对时序信息关注的T-Grpo.
 ![alt text](image-9.png)
T-Grpo将数据分为两组, 一组将视频帧随机shuffle, 分别送给Policy model输出回答, 若顺序组正确率大于shuffle组则顺序组中回答正确的采样获得奖励. 
